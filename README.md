...
1. **Once:** import the whole client list (paste from WhatsApp, or upload Excel / Word / CSV / text). Press **Locate all** so every client is on the map. Type the **Starting point** (where the boys load and start) at the top of the plan and press Locate. Under *Setup*, add the delivery boys' names and tick "by Porter" on box sizes too big for a bike. All of this is remembered. Two clients with the same name at different addresses are simply two entries; the address is shown under the name.
